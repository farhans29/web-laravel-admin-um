...
class CustomerController extends Controller
{
    ...
    private function getCustomersQuery($search = null, $registrationStatus = null, $propertyId = null, $bookingStatus = null)
    {
        ...
        $registeredUsers = User::select([
            'users.id',
            DB::raw('users.username COLLATE utf8mb4_unicode_ci as username'),
            DB::raw('users.email COLLATE utf8mb4_unicode_ci as email'),
            DB::raw('users.phone_number COLLATE utf8mb4_unicode_ci as phone'),
            DB::raw('"registered" COLLATE utf8mb4_unicode_ci as registration_status'),
            ...
            DB::raw('(SELECT property_name COLLATE utf8mb4_unicode_ci FROM t_transactions t2 WHERE t2.user_id = users.id ORDER BY t2.transaction_date DESC LIMIT 1) as last_property_name'),
            DB::raw('(SELECT room_name COLLATE utf8mb4_unicode_ci FROM t_transactions t3 WHERE t3.user_id = users.id ORDER BY t3.transaction_date DESC LIMIT 1) as last_room_name'),
            DB::raw('(SELECT m_rooms.no COLLATE utf8mb4_unicode_ci FROM t_transactions t4 LEFT JOIN m_rooms ON t4.room_id = m_rooms.idrec WHERE t4.user_id = users.id ORDER BY t4.transaction_date DESC LIMIT 1) as last_room_number'),
            DB::raw('(SELECT renewal_status COLLATE utf8mb4_unicode_ci FROM t_transactions t5 WHERE t5.user_id = users.id ORDER BY t5.transaction_date DESC LIMIT 1) as renewal_status'),
            ...
            DB::raw('(SELECT GROUP_CONCAT(DISTINCT CONCAT(parking_type, " (", vehicle_plate, ")") SEPARATOR ", ") COLLATE utf8mb4_unicode_ci FROM t_parking_fee_transaction pft WHERE pft.user_id = users.id AND pft.status = 1) as parking_info'),
            DB::raw('(SELECT t_booking.status COLLATE utf8mb4_unicode_ci FROM t_booking INNER JOIN t_transactions t7 ON BINARY t_booking.order_id = BINARY t7.order_id WHERE t7.user_id = users.id ORDER BY t7.transaction_date DESC LIMIT 1) as current_booking_status'),
        ...
        $guestCustomers = Transaction::select([
            DB::raw('NULL as id'),
            DB::raw('user_name COLLATE utf8mb4_unicode_ci as username'),
            DB::raw('user_email COLLATE utf8mb4_unicode_ci as email'),
            DB::raw('user_phone_number COLLATE utf8mb4_unicode_ci as phone'),
            DB::raw('"guest" COLLATE utf8mb4_unicode_ci as registration_status'),
            ...
            DB::raw('(SELECT property_name COLLATE utf8mb4_unicode_ci FROM t_transactions t2 WHERE BINARY t2.user_email = BINARY t_transactions.user_email AND t2.user_id IS NULL ORDER BY t2.transaction_date DESC LIMIT 1) as last_property_name'),
            DB::raw('(SELECT room_name COLLATE utf8mb4_unicode_ci FROM t_transactions t3 WHERE BINARY t3.user_email = BINARY t_transactions.user_email AND t3.user_id IS NULL ORDER BY t3.transaction_date DESC LIMIT 1) as last_room_name'),
            DB::raw('(SELECT m_rooms.no COLLATE utf8mb4_unicode_ci FROM t_transactions t4 LEFT JOIN m_rooms ON t4.room_id = m_rooms.idrec WHERE BINARY t4.user_email = BINARY t_transactions.user_email AND t4.user_id IS NULL ORDER BY t4.transaction_date DESC LIMIT 1) as last_room_number'),
            DB::raw('(SELECT renewal_status COLLATE utf8mb4_unicode_ci FROM t_transactions t5 WHERE BINARY t5.user_email = BINARY t_transactions.user_email AND t5.user_id IS NULL ORDER BY t5.transaction_date DESC LIMIT 1) as renewal_status'),
            ...
            DB::raw('(SELECT GROUP_CONCAT(DISTINCT CONCAT(parking_type, " (", vehicle_plate, ")") SEPARATOR ", ") COLLATE utf8mb4_unicode_ci FROM t_parking_fee_transaction pft WHERE BINARY pft.user_phone = BINARY t_transactions.user_phone_number AND pft.status = 1) as parking_info'),
            DB::raw('(SELECT t_booking.status COLLATE utf8mb4_unicode_ci FROM t_booking INNER JOIN t_transactions t7 ON BINARY t_booking.order_id = BINARY t7.order_id WHERE BINARY t7.user_email = BINARY t_transactions.user_email AND t7.user_id IS NULL ORDER BY t7.transaction_date DESC LIMIT 1) as current_booking_status'),
